...
test.group('Mixins.Ordered.modeAfter', (group) => {
  group.setup(async () => {
  ...
  test('should move model after another in the sequence', async ({ assert }) => {
    ...
    await orderedModels[0].moveAfter(orderedModels[1])
  ...
  test('should move model after first item in the sequence', async ({ assert }) => {
    class Ordered extends compose(BaseModel, OrderedMixin('ordered')) {
      public static table = 'ordered'
      @column({ isPrimary: true })
      public id: number
    ...
    await orderedModels[2].moveAfter(orderedModels[0])
  ...
  test('should move model after last item in the sequence', async ({ assert }) => {
    class Ordered extends compose(BaseModel, OrderedMixin('ordered')) {
      public static table = 'ordered'
      @column({ isPrimary: true })
      public id: number
    ...
    await orderedModels[0].moveAfter(orderedModels[2])
